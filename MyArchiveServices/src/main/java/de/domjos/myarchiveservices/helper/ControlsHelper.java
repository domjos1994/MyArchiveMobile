...
public final class ControlsHelper {
    ...
    public static List<BaseDescriptionObject> getAllMediaItems(Context context, MediaFilter mediaFilter, String extendedWhere, String key, Database database, int mediaCount, int offset, String orderBy) {
        String where = "";

        if(mediaFilter.isList()) {
            try {
                List<MediaList> mediaLists = database.getMediaLists("id=" + mediaFilter.getMediaList().getId(), mediaCount, offset);
    ...
    private static List<BaseDescriptionObject> getAllMediaItems(Context context, Map<DatabaseObject, String> mp, Database database, int mediaCount, int offset, String orderBy) {
        List<BaseDescriptionObject> baseDescriptionObjects = new LinkedList<>();
        for(BaseMediaObject baseMediaObject : database.getObjectList(mp, mediaCount, offset, orderBy)) {
            if(baseMediaObject instanceof Book) {
