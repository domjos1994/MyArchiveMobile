...
public class TreeViewTask extends CustomStatusTask<Void, TreeNode> {
        ...
        try {
            ...
            if(treeNode == null) {
                ...
                if(this.checkDatabase) {
                    for(Person person : this.database.getPersons("")) {
                        List<TreeFile> treeFiles = this.database.getTreeNodeFiles("internalId=" + person.getId() + " AND internalTable='persons' AND internalColumn='image'");
                    ...
                    status = 0;
                    for(Company company : this.database.getCompanies("")) {
                        List<TreeFile> treeFiles = this.database.getTreeNodeFiles("internalId=" + company.getId() + " AND internalTable='companies' AND internalColumn='cover'");
                        if(treeFiles.isEmpty()) {
                    ...
                    status = 0;
                    for(Book book : this.database.getBooks("", -1, 0)) {
                        List<TreeFile> treeFiles = this.database.getTreeNodeFiles("internalId=" + book.getId() + " AND internalTable='books' AND internalColumn='cover'");
                        if(treeFiles.isEmpty()) {
                    ...
                    status = 0;
                    for(Movie movie : this.database.getMovies("", -1, 0)) {
                        List<TreeFile> treeFiles = this.database.getTreeNodeFiles("internalId=" + movie.getId() + " AND internalTable='movies' AND internalColumn='cover'");
                        if(treeFiles.isEmpty()) {
                    ...
                    status = 0;
                    for(Album album : this.database.getAlbums("", -1, 0)) {
                        List<TreeFile> treeFiles = this.database.getTreeNodeFiles("internalId=" + album.getId() + " AND internalTable='albums' AND internalColumn='cover'");
                        if(treeFiles.isEmpty()) {
                    ...
                    status = 0;
                    for(Game game : this.database.getGames("", -1, 0)) {
                        List<TreeFile> treeFiles = this.database.getTreeNodeFiles("internalId=" + game.getId() + " AND internalTable='games' AND internalColumn='cover'");
                        if(treeFiles.isEmpty()) {
    ...
    private void addChildren(de.domjos.myarchivelibrary.model.media.fileTree.TreeNode dbRoot, TreeNode viewRoot) {
        for(de.domjos.myarchivelibrary.model.media.fileTree.TreeNode dbChild : dbRoot.getChildren()) {
